...
lumi = 59.74*1000
print("opening")
def openReco2(name,xsec):
  file = uproot.open("../root/%s"%name)
  t= file["TreeMaker2/PreSelection"]
  df = t.arrays([
      "Shape_sphericity_boosted","Shape_sphericity",
      "Shape_C_boosted","Shape_C",
      "Shape_D_boosted","Shape_D",
      "Shape_circularity_boosted","Shape_circularity",
      "Shape_aplanarity_boosted","Shape_aplanarity",
      "Rho0_05","Rho1_05","Rho2_05",
      "Rho0_10","Rho1_10","Rho2_10",
      "Rho0_20","Rho1_20","Rho2_20",
      "Tau21","Tau32",
      "Jet_suep_pt","Jet_suep_eta","Jet_suep_phi","Jet_suep_mass","Jet_suep_nTracks",
      "Jet_isr_pt","Jet_isr_eta","Jet_isr_phi","Jet_isr_mass","Jet_isr_nTracks"
      ],library="pd")
  df["wgt"] = xsec*lumi/10000 #maybe instead get length (nevents) and divide by that instread of the set value
  df = df.rename(columns={"Jet_suep_nTracks":"jetTracks"})
  return df

df1000 =openReco2("PrivateSamples.SUEP_2018_mMed-1000_mDark-2_temp-2_decay-darkPho_13TeV-pythia8_n-100_0_RA2AnalysisTree.root",0.17)
qcd_df =openReco2("Autumn18.QCD_HT1500to2000_TuneCP5_13TeV-madgraphMLM-pythia8_3_RA2AnalysisTree.root",119.9/10)
print(df1000)
print(qcd_df)
#
...
SR_rhos2.append(mp.Process(target=make_2d_SR, args=(df1000, "jetTracks",range(0,500), "Shape_sphericity",tau_bins,qcd_df,1000,tracksel,spheresel)))
SR_rhos2.append(mp.Process(target=make_2d_SR, args=(df1000, "jetTracks",range(0,500), "Shape_sphericity_boosted",tau_bins,qcd_df,1000,tracksel,spheresel)))
SR_rhos2.append(mp.Process(target=make_2d_SR, args=(df1000, "jetTracks",range(0,500), "Shape_C",tau_bins,qcd_df,1000,tracksel,spheresel)))
SR_rhos2.append(mp.Process(target=make_2d_SR, args=(df1000, "jetTracks",range(0,500), "Shape_C_boosted",tau_bins,qcd_df,1000,tracksel,spheresel)))
SR_rhos2.append(mp.Process(target=make_2d_SR, args=(df1000, "jetTracks",range(0,500), "Shape_D",tau_bins,qcd_df,1000,tracksel,spheresel)))
SR_rhos2.append(mp.Process(target=make_2d_SR, args=(df1000, "jetTracks",range(0,500), "Shape_D_boosted",tau_bins,qcd_df,1000,tracksel,spheresel)))
SR_rhos2.append(mp.Process(target=make_2d_SR, args=(df1000, "jetTracks",range(0,500), "Shape_aplanarity",tau_bins,qcd_df,1000,tracksel,spheresel)))
SR_rhos2.append(mp.Process(target=make_2d_SR, args=(df1000, "jetTracks",range(0,500), "Shape_aplanarity_boosted",tau_bins,qcd_df,1000,tracksel,spheresel)))
