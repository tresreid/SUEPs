...
def trk_var(df,qcd_df,name,idtype):
  ...
  #partType = {0:"all",11:"electron",13:"muon",22:"photon",211:"pion",100:"hadron",1:"pv >= 2",2:"pv >=2; matched==1",3:"pv >= 2; matched==1; quality > 0"}
  ...
  ax[0,0].hist(df[df["min_dR"] < 0.05]["trk_pv"]      ,bins=4,range=[0,4],histtype=u'step',weights=df[df["min_dR"] < 0.05]["wgt"],color="blue",label="true")
  ax[0,0].hist(df[df["min_dR"] >= 0.05]["trk_pv"]     ,bins=4,range=[0,4],histtype=u'step',weights=df[df["min_dR"] >= 0.05]["wgt"],color="red",label="false")
  ax[0,0].hist(qcd_df["trk_pv"]                       ,bins=4,range=[0,4],histtype=u'step',weights=qcd_df["wgt"],color="black",label="qcd")
  ax[1,0].hist(df[df["min_dR"] < 0.05]["trk_quality"] ,bins=8,range=[-2,6],histtype=u'step',weights=df[df["min_dR"] < 0.05]["wgt"],color="blue",label="true")
  ax[1,0].hist(df[df["min_dR"] >= 0.05]["trk_quality"],bins=8,range=[-2,6],histtype=u'step',weights=df[df["min_dR"] >= 0.05]["wgt"],color="red",label="false")
  ax[1,0].hist(qcd_df["trk_quality"]                  ,bins=8,range=[-2,6],histtype=u'step',weights=qcd_df["wgt"],color="black",label="qcd")
  ax[0,1].hist(df[df["min_dR"] < 0.05]["trk_matched"] ,bins=2,range=[0,2],histtype=u'step',weights=df[df["min_dR"] < 0.05]["wgt"],color="blue",label="true")
  ax[0,1].hist(df[df["min_dR"] >= 0.05]["trk_matched"],bins=2,range=[0,2],histtype=u'step',weights=df[df["min_dR"] >= 0.05]["wgt"],color="red",label="false")
  ax[0,1].hist(qcd_df["trk_matched"]                  ,bins=2,range=[0,2],histtype=u'step',weights=qcd_df["wgt"],color="black",label="qcd")
  ax[1,1].hist(df[df["min_dR"] < 0.05]["trk_chi2"]    ,bins=10,range=[0,200],histtype=u'step',weights=df[df["min_dR"] < 0.05]["wgt"],color="blue",label="true")
  ax[1,1].hist(df[df["min_dR"] >= 0.05]["trk_chi2"]   ,bins=10,range=[0,200],histtype=u'step',weights=df[df["min_dR"] >= 0.05]["wgt"],color="red",label="false")
  ax[1,1].hist(qcd_df["trk_chi2"]                     ,bins=10,range=[0,200],histtype=u'step',weights=qcd_df["wgt"],color="black",label="qcd")
...
#  fig.clear()
  ...
  ax[0,0].hist(df[df["min_dR"] < 0.05]["trk_foundHits"], bins=50,range=[0,50],histtype=u'step',weights=df[df["min_dR"] < 0.05]["wgt"],color="blue",label="true")
  ax[0,0].hist(df[df["min_dR"] >= 0.05]["trk_foundHits"],bins=50,range=[0,50],histtype=u'step',weights=df[df["min_dR"] >= 0.05]["wgt"],color="red",label="false")
  ax[0,0].hist(qcd_df["trk_foundHits"],                  bins=50,range=[0,50],histtype=u'step',weights=qcd_df["wgt"],color="black",label="qcd")
  ax[1,0].hist(df[df["min_dR"] < 0.05]["trk_lostHits"],  bins=5,range=[-1,1],histtype=u'step',weights=df[df["min_dR"] < 0.05]["wgt"],color="blue",label="true")
  ax[1,0].hist(df[df["min_dR"] >= 0.05]["trk_lostHits"], bins=5,range=[-1,1],histtype=u'step',weights=df[df["min_dR"] >= 0.05]["wgt"],color="red",label="false")
  ax[1,0].hist(qcd_df["trk_lostHits"],                   bins=5,range=[-1,1],histtype=u'step',weights=qcd_df["wgt"],color="black",label="qcd")
  ax[0,1].hist(df[df["min_dR"] < 0.05]["trk_nHits"],     bins=50,range=[0,50],histtype=u'step',weights=df[df["min_dR"] < 0.05]["wgt"],color="blue",label="true")
  ax[0,1].hist(df[df["min_dR"] >= 0.05]["trk_nHits"],    bins=50,range=[0,50],histtype=u'step',weights=df[df["min_dR"] >= 0.05]["wgt"],color="red",label="false")
  ax[0,1].hist(qcd_df["trk_nHits"],                      bins=50,range=[0,50],histtype=u'step',weights=qcd_df["wgt"],color="black",label="qcd")
  ax[1,1].hist(df[df["min_dR"] < 0.05]["trk_nPHits"],    bins=12,range=[0,12],histtype=u'step',weights=df[df["min_dR"] < 0.05]["wgt"],color="blue",label="true")
  ax[1,1].hist(df[df["min_dR"] >= 0.05]["trk_nPHits"],   bins=12,range=[0,12],histtype=u'step',weights=df[df["min_dR"] >= 0.05]["wgt"],color="red",label="false")
  ax[1,1].hist(qcd_df["trk_nPHits"],                     bins=12,range=[0,12],histtype=u'step',weights=qcd_df["wgt"],color="black",label="qcd")
...
def make_eff(df,dfID,gen_df,var,bins,idtype,sample):
  ...
  #eff["qcdeff"] = (qcdreco_group/qcdgen_group)
  #eff["qcdyerrx"]=(qcdreco_group/qcdgen_group)*np.sqrt((1/qcdreco_group)+(1/qcdgen_group))
  ...
  #ax[1,0].errorbar(eff["range"].to_numpy()+bins[:-1],eff["recoeff"].to_numpy(),eff["reco_yerr"].to_numpy(),xerr=eff["range"],ls='none',label="reco eff",color="black")
  #ax[1,0].errorbar(eff["range"].to_numpy()+bins[:-1],eff["IDeff"].to_numpy(),eff["ID_yerr"].to_numpy(),xerr=eff["range"],ls='none',label="ID eff",color="green")
  #ax[1,0].errorbar(eff["range"].to_numpy()+bins[:-1],eff["fakeRate"].to_numpy(),eff["fake_yerr"].to_numpy(),xerr=eff["range"],ls='none',label="fake Rate",color="red")
  #ax[1,0].errorbar(eff["range"].to_numpy()+bins[:-1],eff["purity"].to_numpy(),eff["pure_yerr"].to_numpy(),xerr=eff["range"],ls='none',label="purity",color="blue")
  #ax[1,0].errorbar(eff["range"].to_numpy()+bins[:-1],eff["eff"].to_numpy(),eff["yerrx"].to_numpy(),xerr=eff["range"],ls='none',label="signal",color="blue")
  #ax[1,1].errorbar(eff["range"].to_numpy()+bins[:-1],eff["qcdeff"].to_numpy(),eff["qcdyerrx"].to_numpy(),xerr=eff["range"],ls='none',label="qcd",color="black")
  #ax[1,0].errorbar(eff["range"].to_numpy()+bins[:-1],eff["qcdeff"].to_numpy(),eff["qcdyerrx"].to_numpy(),xerr=eff["range"],ls='none',label="qcd",color="black")
...
plot_id=0
def make_eff_combo(reco_id,qcd_id,gen_df,idx,binx):
  global plot_id
...
#    ax.plot([],[],' ',label="max sig: %.2f"%(np.max((sig/(np.sqrt(np.add(sig,bkg)))))))
    ...
    ax.hist(reco_group["nTracks"],  bins=binx,histtype=u'step',range=[0,binx*10],weights=reco_group["wgt"],color=colors[i],linestyle="solid",label="sig:pt>%.2f"%(pt[i]))
    ax.hist(qcd_group["nTracks"],   bins=binx,histtype=u'step',range=[0,binx*10],weights=qcd_group["wgt"],color=colors[i],linestyle="dashed",label="qcd:pt>%.2f"%(pt[i]))
    ax2.errorbar(range(0,binx*10,10),sig/(np.sqrt(np.add(sig,bkg))),(sig/(np.sqrt(np.add(sig,bkg))))*np.sqrt(np.add(np.reciprocal(sig),np.reciprocal(4*np.add(sig,bkg)))),ecolor=colors[i],label="signif: pt>%.2f"%(pt[i]),color=colors[i],linestyle="dashdot",errorevery=10)
  ...
  plot_id = plot_id + 1
  fig.tight_layout()
  fig.savefig("Plots/nTracks/nTracks_%s.png"%(plot_id))
  plt.close()
...
make_eff(df1000,df1000_pmt,gen_df1000[gen_df1000["pt"]>1],"pt",pt_bins,4,"sig1000")
make_eff(df1000,df1000_pmt,gen_df1000[gen_df1000["pt"]>1],"eta",eta_bins,4,"sig1000")
make_eff(df1000,df1000_pmt,gen_df1000[gen_df1000["pt"]>1],"phi",phi_bins,4,"sig1000")
...
make_eff(df200,df200_pmt,gen_df200[gen_df200["pt"]>1],"pt",pt_bins,4,"sig200")
make_eff(df200,df200_pmt,gen_df200[gen_df200["pt"]>1],"eta",eta_bins,4,"sig200")
make_eff(df200,df200_pmt,gen_df200[gen_df200["pt"]>1],"phi",phi_bins,4,"sig200")
...
#trk_var(df1000_pmq,qcd_pmq,"sig1000",3)
...
#trk_var(df200_pmq,qcd_pmq,"sig200",3)
print("nTracks") 
make_eff_combo(df1000_pm,qcd_pm,gen_df1000,"sig1000",55)
make_eff_combo(df200_pm,qcd_pm,gen_df200,"sig200",25)
