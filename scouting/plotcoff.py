...
labels = {"sig1000":r"$m_{\phi}$ = 1000 GeV","sig750":r"$m_{\phi}$ = 750 GeV","sig400":r"$m_{\phi}$ = 400 GeV","sig300":r"$m_{\phi}$ = 300 GeV","sig200":r"$m_{\phi}$ = 200 GeV","RunA":"Data(1%)","QCD":"QCD","Data":"Data(1%)","Trigger":"Trigger Data (100%)"}
...
def make_overlapdists(samples,var,cut,xlab=None,make_ratio=True,vline=None):
  ...
  if "RunA" in samples:
    ...
    ax.errorbar(sdat[1][:-1],sdat[0],yerr=daterr,color=sigcolors["RunA"],label=labels["Data"],marker=".",zorder=0,ls="None")
  if "QCD" in samples:
...
def make_dists(sample,runPV=0):
  ...
  with open(directory+"myhistos_%s.p"%sample, "rb") as pkl_file:
      ...
      scaled = {}
      for name, h in out.items():
        if "SR" in name or "trkID" in name or "2d" in name:
          continue
        if name in skip:
          continue
        print(name)
...
def make_multitrigs(sig,varsx):
  ...
  for i,var in enumerate(varsx):
    with open(directory+"myhistos_%s.p"%sig, "rb") as pkl_file:
        ...
        if("ht" in var):
          s = s.rebin("v1",hist.Bin("v1","ht",[*range(0,700,10)]+[700,800,1000,1200,1500]))
          s0 = s0.rebin("v1",hist.Bin("v1","ht",[*range(0,700,10)]+[700,800,1000,1200,1500]))
  ...
  if("Data" in samples):
    ...
    for sample in samples:
      with open(directory+"myhistos_%s.p"%sample, "rb") as pkl_file:
          ...
          if("ht" in var):
            s = sx.copy().rebin("v1",hist.Bin("v1","ht",[*range(0,700,10)]+[700,800,1000,1200,1500]))
            s0 = s0x.copy().rebin("v1",hist.Bin("v1","ht",[*range(0,700,10)]+[700,800,1000,1200,1500]))
          else:
            s = sx
            s0 = s0x
          s1 = s.to_hist().to_numpy()[0]
...
def make_n1(samples,var,cut,maxpoints,xlab=None):
  ...
  if large_max:
    ...
    ax1.set_ylim([0,1])
  ax.autoscale(axis='y', tight=True)
  fig.savefig("Plots/signif_%s_%s"%(var,cut))
  plt.close()
  ...
  with open(directory+"myhistos_%s_2.p"%sample, "rb") as pkl_file:
      ...
      for name, h in out.items():
        ...
        if isinstance(h, hist.Hist):
          ...
          plt.close()
#def makeSRSignig9(sample,var,cut):
#  A = 1
#  B = 1
#  C = 1
#  D =1
...
#  SR =1
#
#  error = np.sqrt((2*H_err)**2 +(2*D_err)**2 +(2*B_err)**2 +(2*F_err)**2 +(2*G_err)**2 +(2*C_err)**2 +(2*A_err)**2 + (4*E_err)**2)
  ...
  with open(directory+"myhistos_%s_2.p"%sample, "rb") as pkl_file:
      ...
      for name, h in out.items():
        ...
        if isinstance(h, hist.Hist):
          ...
          labsy = [2*int(item.get_text()) for item in ax1.get_yticklabels()]
          ax1.set_xticklabels(labsx)
          ax1.set_yticklabels(labsy)
          ax1.set_xlabel("Ht GeV")
          ax1.set_ylabel("Sphericity (Unboosted)")
          fig.suptitle("Trigger Efficiency")
          ax1.autoscale(axis='y', tight=True)
          ax1.autoscale(axis='x', tight=True)
          hep.cms.label('',data=False,lumi=lumi/1000,year=2018,loc=2)
          fig.savefig("Plots/trigeff2d")
          plt.close()
...
make_overlapdists(["QCD","RunA","sig1000","sig400","sig200"],"FatJet_pt",3,"FatJet pT [GeV]")
make_overlapdists(["QCD","RunA","sig1000","sig400","sig200"],"FatJet_eta",3,"FatJet Eta")
make_overlapdists(["QCD","RunA","sig1000","sig400","sig200"],"FatJet_phi",3,"FatJet Phi")
make_overlapdists(["QCD","RunA","sig1000","sig400","sig200"],"FatJet_ncount50",3,"FatJet(50) Multiplicity")
...
make_n1(["sig1000","sig750","sig400","sig300","sig200"],"FatJet_nconst",3,empty) 
make_n1(["sig1000","sig750","sig400","sig300","sig200"],"FatJet_nconst",4,empty) 
make_n1(["sig1000","sig750","sig400","sig300","sig200"],"sphere1_suep",3,empty)
make_n1(["sig1000","sig750","sig400","sig300","sig200"],"sphere1_suep",4,empty)
