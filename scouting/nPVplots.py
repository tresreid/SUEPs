...
def packdist(output,vals,var):
        ...
        output["dist_%s"%(var)].fill(cut="cut 3-1:nPV < 30", v1=vals[3][var])
        output["dist_%s"%(var)].fill(cut="cut 3-2:nPV >= 30", v1=vals[4][var])
#        if("PFcand_n" in var):
...
def packdistflat(output,vals,var,prefix=""):
        ...
        output["dist_%s%s"%(prefix,var)].fill(cut="cut 3-1:nPV < 30",        v1=ak.flatten(vals[3][var]))
        output["dist_%s%s"%(prefix,var)].fill(cut="cut 3-1:nPV >= 30",        v1=ak.flatten(vals[4][var]))
    ...
    def __init__(self):
        self._accumulator = processor.dict_accumulator({
            ...
            "dist_Vertex_minZ": hist.Hist(
                      "Events",
                      hist.Cat("cut","Cutflow"),
                      hist.Bin("v1","Vertex min dz(PV,closest)",100,0,10)
            ),
...
#          #  ),
    ...
    def process(self, arrays):
        ...
        vals0 = ak.zip({
               ...
               'nPVs_good5': ak.count(arrays["Vertex_tracksSize"][arrays["Vertex_tracksSize"] >10],axis=-1),
               'nPVs_good6': ak.count(arrays["Vertex_tracksSize"][arrays["Vertex_tracksSize"] >15],axis=-1),
               'nPVs_good7': ak.count(arrays["Vertex_tracksSize"][arrays["Vertex_tracksSize"] >25],axis=-1),
...
#       # print(len(alldRtracks))
        ...
        #    #IRM_cands2 = IRM_cands[onetrackcut]
  

        #    #eigs = sphericity(self,bCands,2.0)
        #    #spherex0 = vals0[vals0.FatJet_ncount30 >= 2]
        #    print(len(IRM_cands),len(spherex0))
        #    #print(len(IRM_cands),len(IRM_cands2),len(spherex0))
       ...
       ############################################################################## 
        ...
        vals3 = vals2[vals2.n_pvs < 30]
        vals4 = vals2[vals2.n_pvs >= 30]
        ...
        vals_jet3 = vals_jet2[vals2.n_pvs < 30]
        vals_jet4 = vals_jet2[vals2.n_pvs >= 30]
        ...
        vals_fatjet3 = vals_fatjet2[vals2.n_pvs < 30]
        vals_fatjet4 = vals_fatjet2[vals2.n_pvs >= 30]
        ...
        vals_vertex3 = vals_vertex2[vals2.n_pvs < 30]
        #vals_vertex3 = vals_vertex2[vals2.FatJet_ncount50 >= 2]
        vals_vertex4 = vals_vertex2[vals2.n_pvs >= 30]
...
if __name__ == "__main__":
    ...
    with open("outhists/nPV/myhistos_%s_%s.p"%(fin,batch), "wb") as pkl_file:
